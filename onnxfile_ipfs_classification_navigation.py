...
class ONNXInference:
    def __init__(self, model_directory):
        ...
        # self._tag_sub = rospy.Subscriber("tag_environment", String, self.tag_callback)
        self._pose_sub = rospy.Subscriber("robot_distance_angle", PoseStamped, self.pose_callback)
        self._image_sub = rospy.Subscriber("camera_image", Image, self.image_callback)
        self._reach_sub = rospy.Subscriber("target_reach", String, self.reach_callback)
        self._image_sub = rospy.Subscriber("classification_env", String, self.env_callback)

        self._twist_pub = rospy.Publisher("cmd_vel", Twist, queue_size=1)
        # タグ管理
        self.env_received = threading.Event()  # タグを受信したことを通知するためのイベント
        self.envName = None
        # モデル管理
    ...
    def env_callback(self, env_msg):
        self.envName = env_msg.data
        print(self.envName)
        self.env_received.set()
        
    def get_onnx_hash(self):
        self.env_received.wait()
        ...
        onnx_hash = contract.functions.getHash(self.envName).call({'from': self.default_account})
        print('onnx ipfs hash check : ', onnx_hash)
        do_transact = contract.functions.getHash(self.envName).transact({'from': self.default_account})
        ...
        self.onnx_filename = f'downloaded_model_{self.envName}.onnx'
    ...
    def inference(self):
        ...
        if self.envName in ["Park", "City"]:
            if self.envName not in self.inferenceSession_dict:
                # print('download twist', twist)
                self.file_delete.set()
                print('request ', self.envName, ' onnx model')
                zero_vel = Twist()
                zero_vel.linear.x = 0
                zero_vel.linear.y = 0
                zero_vel.linear.z = 0
                ...
                self.modelName_dict[self.envName] = self.onnx_filename
                print("-------------dict info---------------")
                print('modelName_dict : ', self.modelName_dict)
                session= onnxruntime.InferenceSession(self.onnx_filename)
                self.inferenceSession_dict[self.envName] = session
            ...
            # print('onnx model name : ', self.model_dict[self.envName])
            session = self.inferenceSession_dict[self.envName]
            # print('set onnx model : ', self.modelName_dict[self.envName])
            ...
            # print(f'Unsupported tag name: {self.envName}')
            twist = Twist()
            # print('Returning zero twist:', twist)
            return twist
